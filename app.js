/* =========================================================
   Nexus Transport PR — app.js (PRODUCCIÓN FIXED)
   - Firebase Auth (Google) + Firestore
   - PIN (hash SHA-256)
   - Roles: admin / driver
   - Driver: solo escribe + ve su historial
   - Admin: usuarios, clientes, settings, servicios, cierres, facturar + PDFs
   - FIX: NO ejecutar dashboard/queries sin S.user + sin "entered"
   - Bootstrap admin por UID maestro
   ========================================================= */
...
(() => {
  ...
  const C = {
    USERS: "users",
    INVITES: "invites",     // docId = email en minúscula
    CLIENTS: "clients",
    SERVICES: "services",
  ...
  const DEFAULT_SETTINGS = {
    ...
    connectPct: 0.15,      // -15% Connect
    companyPct: 0.30,      // Empresa 30%
    retentionPct: 0.10     // Retención chofer
  };
  ...
  const S = {
    ...
    profile: null,
    role: null,
    entered: false,     // 🔥 CLAVE: NO render hasta true
  ...
  function setShellTopbar(visible){
    const tb = $("topbar");
  ...
  $$("#navTabs .tab[data-view]").forEach(btn=>{
    btn.addEventListener("click", async ()=>{
      ...
      if (!S.entered) {
        // no navega si no has validado PIN
        setView("login");
        return;
      }
  ...
  async function loadSettings(force=false){
    ...
    // UI brand
    safeSetText("brandName", S.settings.brand || DEFAULT_SETTINGS.brand);
    safeSetText("brandNameTop", S.settings.brand || DEFAULT_SETTINGS.brand);
  }
  ...
  auth.onAuthStateChanged(async (user)=>{
    ...
    safeSetText("pinError", "");
    safeSetText("pinSetError", "");
    safeSetText("loginMsg", "—");

    if (!user){
      S.user=null; S.profile=null; S.role=null;
      setShellTopbar(false);
      setView("login");
      safeSetText("loginMsg", "Inicia sesión con Google");
      return;
    }

    S.user = user;

    // Mientras no "entered", siempre mostramos login y topbar OFF
    setShellTopbar(false);
    setView("login");
    safeSetText("loginMsg", "Google OK. Validando acceso...");

    const uref = db.collection(C.USERS).doc(user.uid);
    let usnap = await uref.get();

    // ✅ Bootstrap Admin por UID maestro
    if (!usnap.exists && user.uid === MASTER_ADMIN_UID) {
      await uref.set({
      ...
      safeSetText("loginMsg", "Admin maestro creado. Crea tu PIN.");
    }
    ...
    if (!usnap.exists){
      ...
      if (!email) {
        safeSetText("loginMsg", "Email no disponible. Reintenta.");
        return;
      }

      const inv = await db.collection(C.INVITES).doc(email).get();
      if (!inv.exists){
        safeSetText("loginMsg", "No autorizado. Admin debe invitar tu email.");
        return;
      }

      const invData = inv.data() || {};
      if (invData.active === false){
        safeSetText("loginMsg", "Invitación inactiva.");
        return;
      }
      ...
      safeSetText("loginMsg", "Usuario creado. Ahora crea tu PIN.");
    }
    ...
    // UI header (aunque topbar esté hidden)
    safeSetText("whoLine", `${S.profile.email || user.email || "—"}`);
    safeSetText("rolePill", (S.role || "—").toUpperCase());

    // preload mínimo para selects/preview en login (no render dashboard)
    ...
    if (!S.profile.pinHash){
      safeSetText("loginMsg", "Crea tu PIN (primera vez) y entra.");
    } else {
      safeSetText("loginMsg", "Confirma tu PIN para entrar.");
    }
  });
  ...
  $("btnPinConfirm")?.addEventListener("click", async ()=>{
    ...
    if (!pin) { safeSetText("pinError", "PIN requerido."); return; }

    const actual = await pinHash(S.user.uid, pin);
    if (actual !== (S.profile.pinHash || "")) {
      safeSetText("pinError", "PIN incorrecto.");
      return;
    }
  ...
  $("btnPinSet")?.addEventListener("click", async ()=>{
    ...
    if (!p1 || !p2) { safeSetText("pinSetError", "Completa ambos campos."); return; }
    if (p1 !== p2) { safeSetText("pinSetError", "PIN no coincide."); return; }
    if (p1.length < 4) { safeSetText("pinSetError", "PIN mínimo 4 dígitos."); return; }
  ...
  $("btnQuickSave")?.addEventListener("click", async ()=>{
    if (!S.entered || !S.user || !S.profile) return;
    ...
    $("qsMiles").value = "";
    $("qsNote").value = "";
    safeSetText("quickPreview", "Guardado ✅");
  ...
  ["qsClient","qsMiles"].forEach(id=>{
    $(id)?.addEventListener("input", ()=>{
      ...
      if (!client || miles<=0){ safeSetText("quickPreview", "—"); return; }
      const r = calc(miles, client);
      safeSetText("quickPreview",
        `Bruto ${money(r.bruto)} | Adj ${money(r.connectAdj)} | Neto chofer ${money(r.driverNet)} | Empresa ${money(r.company)}`
      );
    });
  });

  /* =========================
     14) Dashboard render (FIX: guards)
  ========================= */
  async function queryWeekServices(weekISO){
    // 🔥 FIX: no tocar uid si no hay user / no entered
    if (!S.entered || !S.user) return [];
  ...
  async function renderDashboard(){
    ...
    safeSetText("kpiWeekRange", `Semana desde ${wk}`);
    ...
    safeSetText("kpiWeekMiles", miles.toFixed(1));
    safeSetText("kpiWeekServices", String(list.length));
    safeSetText("kpiWeekDriverNet", money(driverNet));
    safeSetText("kpiWeekCompany", money(company));
  ...
  async function renderUsers(){
    if (!S.entered || S.role !== "admin") return;
    await loadUsers(true);

    const tb = $("usersTable");
    if (!tb) return;

    tb.innerHTML = S.users.slice()
      .sort((a,b)=>String(a.email||"").localeCompare(String(b.email||"")))
      .map(u=>`
        <tr>
  ...
  async function buildWeekly(exportPdf){
    ...
    doc.setFont("helvetica","bold"); line("Totales");
    doc.setFont("helvetica","normal");
  ...
  $("btnWipeCache")?.addEventListener("click", ()=>{
    ...
    // OJO: no borro clients/users porque ayuda offline; pero si quieres, descomenta:
  ...
  async function refreshView(view){
    ...
    // adminClose/adminInvoices/adminSettings no requieren render inmediato
    return;
  }
