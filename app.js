/* =========================================================
   Nexus Transport PR — app.js (PRODUCCIÓN)
...
(() => {
  ...
  const C = {
    USERS: "users",
    INVITES: "invites",     // admin invita por email
    CLIENTS: "clients",
    SERVICES: "services",
  ...
  auth.onAuthStateChanged(async (user)=>{
    ...
    setShell(true);
    setView("login");
    $("loginMsg").textContent = "Google OK. Validando acceso...";

    // 1) Autorizar por doc users/{uid}
    const uref = db.collection(C.USERS).doc(user.uid);
    const usnap = await uref.get();

    if (!usnap.exists){
      // 2) Si no existe: permitir onboarding por INVITE (email)
      const email = (user.email || "").toLowerCase();
      if (!email) {
        $("loginMsg").textContent = "Email no disponible. Reintenta.";
        return;
      }

      const inv = await db.collection(C.INVITES).doc(email).get();
      if (!inv.exists){
        $("loginMsg").textContent = "No autorizado. Admin debe invitar tu email.";
        return;
      }

      const invData = inv.data() || {};
      if (invData.active === false){
        $("loginMsg").textContent = "Invitación inactiva.";
        return;
      }

      // Crear user doc (sin pinHash todavía; chofer lo crea en pantalla)
      await uref.set({
        email,
        displayName: user.displayName || "",
        role: invData.role || "driver",
        active: invData.active !== false,
        pinHash: "", // se setea con 'Guardar PIN'
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      }, { merge:true });

      $("loginMsg").textContent = "Usuario creado. Ahora crea tu PIN.";
    }
    ...
    // UI header
    $("whoLine").textContent = `${S.profile.email || user.email || "—"}`;
    ...
    fillClients("drvClient"); // no existe en driver view (historial), pero ok
    fillClients("sClient");
    ...
    // Si no tiene PIN, exigir set PIN
    if (!S.profile.pinHash){
      $("loginMsg").textContent = "Crea tu PIN (primera vez) y entra.";
    } else {
      $("loginMsg").textContent = "Confirma tu PIN para entrar.";
    }

    // Entra cuando confirme PIN
  });
  ...
  $("btnQuickSave")?.addEventListener("click", async ()=>{
    ...
    // Driver & Admin can save as themselves (admin for testing)
  ...
  /* =========================
     15) Driver history (detail/week/month) + PDF
  ========================= */
  ...
  function groupByMonth(rows){
    const map = new Map();
    rows.forEach(r=>{
      const k = String(r.dateISO||"").slice(0,7); // YYYY-MM
      const cur = map.get(k) || { k, count:0, miles:0, net:0 };
      cur.count += 1;
      cur.miles += num(r.miles);
      cur.net   += num(r.driverNet);
  ...
  async function renderServicesAdmin(){
    ...
    // CSV cache in memory for export
  ...
  /* =========================
     19) Admin: Weekly Close (por chofer) + PDF
  ========================= */
  ...
  async function buildInvoice(exportPdf){
    ...
    // Nota: para facturar a terceros normalmente es bruto; si Connect requiere “neteo”, el cliente.connect lo define
    const connectAdj = (client?.connect) ? round2(bruto * num(S.settings.connectPct)) : 0;
  ...
  function prefillSettingsForm(){
    $("brandName").textContent = S.settings.brand || DEFAULT_SETTINGS.brand;
    $("setBrand").value = S.settings.brand ?? DEFAULT_SETTINGS.brand;
    $("setConnect").value = S.settings.connectPct ?? DEFAULT_SETTINGS.connectPct;
    $("setCompany").value = S.settings.companyPct ?? DEFAULT_SETTINGS.companyPct;
    $("setRetention").value = S.settings.retentionPct ?? DEFAULT_SETTINGS.retentionPct;
    $("setFooter").value = S.settings.footer ?? DEFAULT_SETTINGS.footer;
  }
  ...
  async function refreshView(view){
    ...
    // close/invoices just keep UI ready
    return;
  }
