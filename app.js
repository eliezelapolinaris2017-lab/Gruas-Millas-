/* =========================================================
   Nexus Transport PR — app.js (PRODUCCIÓN ESTABLE)
   - Firebase Auth (Google) + Firestore (v8)
   - PIN SHA-256 (hash = sha256(uid + ":" + pin))
   - Roles: admin / driver
   ...
   - jsPDF ALWAYS
   - FIX: No crashea si falta un elemento del DOM (safe setters)
   ========================================================= */
...
(() => {
  ...
  function setAdminTabsVisible(isAdmin){
    const ids = [
      "tabAdminUsers","tabAdminClients","tabAdminServices",
      "tabAdminClose","tabAdminInvoices","tabAdminSettings"
    ];
    ids.forEach(id => {
      const el = $(id);
  ...
  function fillDrivers(selectId){
    ...
    el.innerHTML =
      `<option value="">—</option>` +
      list.map(u => `<option value="${u.uid}">${escapeHtml(u.email || u.uid)}</option>`).join("");
  }
  ...
  auth.onAuthStateChanged(async (user)=>{
    ...
    setShell(true);
    setView("login");
    setText("loginMsg","Google OK. Validando acceso...");

    // autorizar por users/{uid} o invites/{email}
    ...
    if (!usnap.exists){
      ...
      if (!email){
        setText("loginMsg","Email no disponible. Reintenta.");
        return;
      }
      const inv = await db.collection(C.INVITES).doc(email).get();
      if (!inv.exists){
        setText("loginMsg","No autorizado. Admin debe invitar tu email.");
        return;
      }
      const invData = inv.data() || {};
      if (invData.active === false){
        setText("loginMsg","Invitación inactiva.");
        return;
      }
      ...
      setText("loginMsg","Usuario creado. Ahora crea tu PIN.");
    }
    ...
    setText("whoLine", (S.profile.email || user.email || "—"));
    setText("rolePill", (S.role || "—").toUpperCase());
    ...
    // fill selects (si existen en DOM)
    ...
    // defaults de fechas
    if (exists("qsDate")) setValue("qsDate", todayISO());
    if (exists("drvFrom") && !$("drvFrom").value) setValue("drvFrom", new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().slice(0,10));
    if (exists("drvTo") && !$("drvTo").value) setValue("drvTo", todayISO());
    if (exists("sWeek")) setValue("sWeek", mondayOf(todayISO()));
    if (exists("wkStart")) setValue("wkStart", mondayOf(todayISO()));
    if (exists("invWeek")) setValue("invWeek", mondayOf(todayISO()));

    prefillSettingsForm();

    if (!S.profile.pinHash){
      setText("loginMsg","Crea tu PIN (primera vez) y entra.");
    } else {
      setText("loginMsg","Confirma tu PIN para entrar.");
    }
  });
  ...
  $("btnPinConfirm")?.addEventListener("click", async ()=>{
    ...
    if (!pin) { setText("pinError","PIN requerido."); return; }

    const actual = await pinHash(S.user.uid, pin);
    if (actual !== (S.profile.pinHash || "")) {
      setText("pinError","PIN incorrecto.");
      return;
    }
    await afterEnter();
  });
  ...
  $("btnPinSet")?.addEventListener("click", async ()=>{
    ...
    if (!p1 || !p2) { setText("pinSetError","Completa ambos campos."); return; }
    if (p1 !== p2) { setText("pinSetError","PIN no coincide."); return; }
    if (p1.length < 4) { setText("pinSetError","PIN mínimo 4 dígitos."); return; }
  ...
  /* =========================
     12) Engine de cálculo (por milla)
  ========================= */
  ...
  $("btnQuickSave")?.addEventListener("click", async ()=>{
    ...
    const dateISO  = (exists("qsDate") ? ($("qsDate").value || todayISO()) : todayISO());
    const clientId = (exists("qsClient") ? ($("qsClient").value || "") : "");
    const miles    = (exists("qsMiles") ? num($("qsMiles").value) : 0);
    const note     = (exists("qsNote") ? (($("qsNote").value || "").trim()) : "");
    ...
    if (exists("qsMiles")) $("qsMiles").value = "";
    if (exists("qsNote")) $("qsNote").value = "";
    setText("quickPreview","Guardado ✅");
  ...
  $("btnQuickClear")?.addEventListener("click", ()=>{
    if (exists("qsMiles")) $("qsMiles").value = "";
    if (exists("qsNote")) $("qsNote").value = "";
    setText("quickPreview","—");
  });

  ["qsClient","qsMiles"].forEach(id=>{
    $(id)?.addEventListener("input", ()=>{
      const cid = exists("qsClient") ? ($("qsClient").value || "") : "";
      const miles = exists("qsMiles") ? num($("qsMiles").value) : 0;
      const client = S.clients.find(c=>c.id===cid);
      if (!client || miles<=0){ setText("quickPreview","—"); return; }
      const r = calc(miles, client);
      setText("quickPreview", `Bruto ${money(r.bruto)} | Adj ${money(r.connectAdj)} | Neto chofer ${money(r.driverNet)} | Empresa ${money(r.company)}`);
    });
  });

  /* =========================
     14) Dashboard (role-aware)
  ========================= */
  ...
  async function renderDashboard(){
    ...
    setText("kpiWeekRange", `Semana desde ${wk}`);
    ...
    setText("kpiWeekMiles", miles.toFixed(1));
    setText("kpiWeekServices", String(list.length));
    setText("kpiWeekDriverNet", money(driverNet));
    setText("kpiWeekCompany", money(company));
    ...
    tb.innerHTML = list.slice(0,20).map(s=>`
      <tr>
        ...
        <td class="num">
          ${S.role==="admin" ? `<button class="btn danger" data-del="${s.id}">X</button>` : "—"}
        </td>
      </tr>
    `).join("");
  ...
  async function renderDriverHistory(){
    ...
    const from = (exists("drvFrom") ? ($("drvFrom").value || "1900-01-01") : "1900-01-01");
    const to   = (exists("drvTo") ? ($("drvTo").value || todayISO()) : todayISO());
    const mode = (exists("drvGroup") ? ($("drvGroup").value || "none") : "none");
  ...
  $("btnDrvPDF")?.addEventListener("click", async ()=>{
    ...
    const from = exists("drvFrom") ? ($("drvFrom").value || "—") : "—";
    const to   = exists("drvTo") ? ($("drvTo").value || "—") : "—";
  ...
  $("btnInviteUser")?.addEventListener("click", async ()=>{
    ...
    const email = (exists("uEmail") ? ($("uEmail").value || "").trim().toLowerCase() : "");
    const role  = (exists("uRole") ? ($("uRole").value || "driver") : "driver");
    const active = (exists("uActive") ? (($("uActive").value || "true")==="true") : true);
    const note  = (exists("uNote") ? ($("uNote").value || "").trim() : "");
    ...
    if (exists("uEmail")) $("uEmail").value="";
    if (exists("uNote")) $("uNote").value="";
  });
  ...
  function prefillClientForm(c){
    setValue("cName", c?.name || "");
    setValue("cRate", (c?.rate ?? ""));
    setValue("cConnect", c?.connect ? "yes":"no");
    setValue("cActive", (c?.active !== false) ? "true":"false");
  ...
  $("btnSaveClient")?.addEventListener("click", async ()=>{
    ...
    const name = (exists("cName") ? ($("cName").value || "").trim() : "");
    const rate = round2(exists("cRate") ? num($("cRate").value) : 0);
    const connect = (exists("cConnect") ? (($("cConnect").value || "no")==="yes") : false);
    const active = (exists("cActive") ? (($("cActive").value || "true")==="true") : true);
  ...
  async function queryServicesAdmin(){
    const week = exists("sWeek") ? ($("sWeek").value || "") : "";
    const driverUid = exists("sDriver") ? ($("sDriver").value || "") : "";
    const clientId  = exists("sClient") ? ($("sClient").value || "") : "";
  ...
  async function buildWeekly(exportPdf){
    const weekISO = exists("wkStart") ? ($("wkStart").value || mondayOf(todayISO())) : mondayOf(todayISO());
    const driverUid = exists("wkDriver") ? ($("wkDriver").value || "") : "";
  ...
  async function buildInvoice(exportPdf){
    const weekISO = exists("invWeek") ? ($("invWeek").value || mondayOf(todayISO())) : mondayOf(todayISO());
    const clientId = exists("invClient") ? ($("invClient").value || "") : "";
    if (!clientId) return toast("Selecciona cliente.");
  ...
  function prefillSettingsForm(){
    // Marca superior
    setText("brandName", S.settings.brand || DEFAULT_SETTINGS.brand);

    setValue("setBrand", S.settings.brand ?? DEFAULT_SETTINGS.brand);
    setValue("setConnect", S.settings.connectPct ?? DEFAULT_SETTINGS.connectPct);
    setValue("setCompany", S.settings.companyPct ?? DEFAULT_SETTINGS.companyPct);
    setValue("setRetention", S.settings.retentionPct ?? DEFAULT_SETTINGS.retentionPct);
    setValue("setFooter", S.settings.footer ?? DEFAULT_SETTINGS.footer);
  }

  $("btnSaveSettings")?.addEventListener("click", async ()=>{
    if (S.role !== "admin") return;

    const brand = (exists("setBrand") ? (($("setBrand").value||"").trim()) : "") || DEFAULT_SETTINGS.brand;
    const connectPct = exists("setConnect") ? num($("setConnect").value) : DEFAULT_SETTINGS.connectPct;
    const companyPct = exists("setCompany") ? num($("setCompany").value) : DEFAULT_SETTINGS.companyPct;
    const retentionPct = exists("setRetention") ? num($("setRetention").value) : DEFAULT_SETTINGS.retentionPct;
    const footer = (exists("setFooter") ? (($("setFooter").value||"").trim()) : "") || DEFAULT_SETTINGS.footer;
  ...
  async function refreshView(view){
    ...
    // close / invoices / settings: se actualizan por botones, pero dejamos settings precargado
