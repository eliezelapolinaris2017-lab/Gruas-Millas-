/* =========================================================
   Nexus Transport PR — app.js (PRODUCCIÓN / BASE NUEVA)
   - Firebase Auth (Google) + Firestore (v8 CDN)
   - PIN (hash SHA-256)
   - Roles: admin / driver
   - Driver: solo escribe + ve su historial
   - Admin: usuarios, invites, clientes, settings, servicios, cierres, facturar + PDFs
...
(() => {
  ...
  // ===== BOOT CHECK (no negocia) =====
  if (!window.firebase || !firebase.initializeApp) {
    alert(
      "Firebase SDK NO cargó.\n\n" +
      "✅ Confirma que index.html tenga:\n" +
      "- firebase-app.js\n- firebase-auth.js\n- firebase-firestore.js\n\n" +
      "y que app.js esté AL FINAL del body."
    );
    throw new Error("Firebase SDK missing");
  }

  // Init (safe on refresh)
  try {
    firebase.initializeApp(firebaseConfig);
    console.log("✅ Firebase inicializado:", firebase.app().options.projectId);
  } catch (e) {
    if (!/already exists/i.test(String(e))) throw e;
    console.log("ℹ️ Firebase ya estaba inicializado (refresh)");
  }
  ...
  ========================= */
  const CACHE = {
    SETTINGS: "ntpr.settings.cache.v2",
    CLIENTS:  "ntpr.clients.cache.v2",
    USERS:    "ntpr.users.cache.v2"
  };
  ...
  auth.onAuthStateChanged(async (user)=>{
    ...
    // Crear usuario por INVITE si no existe
    if (!usnap.exists){
  ...
  /* =========================
     12) Engine de cálculo (millas * rate)
  ========================= */
  ...
  /* =========================
     13) Driver Quick Save (solo millas + cliente)
  ========================= */
